-- ============================================================
-- Migration 010 — Fix fn_dispatch_sos_atomic RETURNING clause
-- Guardian Dispatch Platform
-- ============================================================
-- Fixes: "column reference incident_code is ambiguous"
-- The RETURNS TABLE definition includes an incident_code OUT
-- parameter which conflicts with the incidents.incident_code
-- column in the INSERT...RETURNING clause. Qualify with table name.
-- ============================================================
...
BEGIN
  ...
  WHERE i.citizen_id = p_citizen_id
    AND i.status IN ('pending', 'acknowledged', 'assigned', 'accepted', 'en_route', 'arrived')
